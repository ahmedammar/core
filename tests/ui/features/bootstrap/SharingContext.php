	...
	public function itShouldNotBePossibleToShare($name) {
		try {
			...
			PHPUnit_Framework_Assert::assertTrue(
				in_array(
					$e->getMessage(), ['could not find share-with-field',
					'could not find sharing button in fileRow']
				),
				'exception message has to be "could not find share-with-field"' .
				' or "could not find sharing button in fileRow" but was: "' .
				$e->getMessage() . '"'
			);
		}
	}
